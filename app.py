...
def match_upload():
    ...
    # Save this match record in matches.json with a defined reason generated by NLP style comparison.
    reason_text = (
        f"The top '{result['new_item']['label']}' complements the bottom '{best_candidate['data']['label']}' "
        f"due to similar style, color and pattern attributes. Similarity score: {best_score:.2f}."
    )
...
def auto_match():
    ...
    for top in tops:
        ...
        if best_bottom:
            reason_text = (
                f"The top '{top['data']['label']}' complements the bottom '{best_bottom['data']['label']}' "
                f"with similar styles. Similarity score: {best_score:.2f}."
            )
            auto_matches.append({
                "top": top,
                "bottom": best_bottom,
